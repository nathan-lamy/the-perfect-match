...
import {
  Plug, Users, Clock4, FolderTree, CalendarRange, Filter,
  ListOrdered, Sparkles, Send, Terminal,
} from "lucide-react";

const STAGES: { id: Stage; label: string; icon: typeof Plug; hint: string }[] = [
  { id: "connect", label: "Connect", icon: Plug, hint: "BJColle session" },
  { id: "students", label: "Students", icon: Users, hint: "Roster + history" },
  { id: "restrictions", label: "Restrictions", icon: Clock4, hint: "Activity conflicts" },
  { id: "slots", label: "Slots", icon: CalendarRange, hint: "Fetch & select" },
...
export function Sidebar({ onOpenDebug }: { onOpenDebug: () => void }) {
  ...
  const completed = (s: Stage): boolean => {
    switch (s) {
      case "connect": return state.isConnected;
      case "students": return state.students.length > 0;
      case "slots": return state.selectedSlotIds.length > 0;
      case "compute": return result !== null;
    ...
    <aside className="flex h-screen w-64 flex-col border-r border-sidebar-border bg-sidebar text-sidebar-foreground">
      <div className="flex items-center gap-2.5 px-5 py-5">
        ...
        <div className="leading-tight">
          <div className="text-sm font-semibold tracking-tight">The Perfect Match</div>
          <div className="font-mono text-[10px] uppercase tracking-wider text-muted-foreground">By Nathan · v2</div>
        </div>
      </div>
      ...
      <div className="mx-3 mb-3 flex items-center gap-2 rounded-md border border-sidebar-border bg-sidebar-accent/30 px-3 py-2">
        <span className={cn(
          ...
          status === "connected" ? "bg-success pulse-dot" : "bg-muted-foreground/50",
        )} />
      ...
      <div className="border-t border-sidebar-border p-3">
        ...
        >
          ...
          <kbd className="ml-auto rounded border border-border bg-surface-2 px-1.5 py-0.5 font-mono text-[10px]">~</kbd>
        </button>
      </div>
    </aside>
  );
